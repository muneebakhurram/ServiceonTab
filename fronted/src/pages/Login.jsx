...
export const LoginPageConsumer = () => {
  ...
  const handleLogin = async (e) => {
    ...
    // Attempting to log in as a provider (after consumer login attempt)
    try {
      const providerResponse = await axios.post('http://localhost:5000/api/providers/login', {
        email,
        password,
      });

      if (providerResponse.data.success) {
        navigate('/serviceproviderdashboard'); // Navigate to Service Provider Dashboard
      } else {
        setErrorMessage("Login failed for provider. Please check your credentials.");
      }
    } catch (providerError) {
      if (providerError.response) {
        setErrorMessage(providerError.response.data.message || "Login failed for provider. Please try again.");
      } else {
        setErrorMessage("Network error. Please try again.");
      }
    }
